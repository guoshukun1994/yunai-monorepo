...
-   pnpm add @proj/react-components --filter @proj/react-x **软连接的过程都会体现在root下的node_modules/.pnpm中**
...
在提交commit时可能会报错： SyntaxErroe: Invalid or unexpected token解决办法：这是因为配置文件的编码格式不正确：更改vscode下方编码格式改成 UTF-

## commit lint

1. 进行 pre-commit 代码规范检测 `pnpm i husky lint-staged -D` package.json 中配置：
...
2. 进行 commit message 检测 `pnpm add commitizen cz-conventional-changelog lint-staged -D -w`
...
把dist包用一个新窗口打开右键html文件 open with live server
...
-   配置 tailwind 基础设施 tailwind.css **想要样式生效还需在.postcssrc.json里加入tailwindcss**
...
1. 解决 markdown 文件 从中间换行的问题在 .prettierrc.json 中添加：

```js
 "proseWrap"："never"
```

2.  解决渲染的 markdown 文件样式丢失问题在 tailwind.config.js 中添加：
